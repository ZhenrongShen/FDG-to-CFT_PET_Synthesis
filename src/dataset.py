...
class PETDataset(Dataset):
    def __init__(self, data_root, data_list, image_size=None, normalize=False):
        ...
        with open(os.path.join(data_root, data_list), "r") as file:
            for line in file:
                ...
                if "_unpaired_" in data_name:
                    ...
                    FDG_img_path = os.path.join(data_root, data_name, f"{patient_id}_FDG.nii").replace('\\', '/')
                    CFT_img_path = os.path.join(data_root, data_name, f"{patient_id}_CFT.nii").replace('\\', '/')
                # check if the file exists
                if not os.path.exists(FDG_img_path):
                    raise ValueError(f"{FDG_img_path} does not exist.")
                if CFT_img_path is not None and not os.path.exists(CFT_img_path):
        ...
        with open(os.path.join(data_root, "unpaired_NC_CFT_data_list.txt"), "r") as file:
            for line in file:
...
def load_and_resize_image(img_path, image_size=None):
    ...
    # resize option 2: resampling (spacings changed)
        ...
        newSize = (image_size, image_size, image_size)
        resampler.SetSize(newSize)
